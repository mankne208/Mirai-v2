...
const dashboard = http.createServer(function (_req, res) {
    ...
    res.write("Xin chào Nguyễn Mank");
    res.end();
});

dashboard.listen(process.env.port || 0);

logger("BOT Mank đã khởi tạo thành công", "BOT Mank đang khởi tạo");
...
function startBot(message) {
    (message) ? logger(message, "BOT Mank đang khởi chạy") : "";
    ...
    child.on("close", (codeExit) => {
        if (codeExit != 0 || global.countRestart && global.countRestart < 5) {
            startBot("BOT Mank đã được restart");
    ...
    child.on("error", function (error) {
        logger("An error occurred: " + JSON.stringify(error), "BOT Mank đang khởi tạo");
    });
};
